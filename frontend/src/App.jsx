...
import { MOCK_CONTRACT_TEXT, MOCK_RESULTS, USERS } from './data'
...
function makeHighlightedText(fullText, clauseId) {
  ...
  return [
    fullText.substring(0, m.index),
    <span key="hl" className="highlight-yellow">{m[0]}</span>,
    fullText.substring(m.index + m[0].length)
  ]
}
...
export default function App() {
  ...
  const saveContract = () => {
    ...
    if (!c) return
    fetch(API + '/api/contracts/save', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: String(activeId), name: c.name, content: contractTextRef.current, status: view, selections, notes, approval_status: c.approval_status || '', approval_id: c.approval_id || '' })
    }).catch(() => { })
  }
  ...
  const handleNewContract = () => {
    ...
    setActiveId(id); setView('upload'); setContractText(''); setFile(null)
  }

  const handleSubmit = () => {
    setView('analyzing')
    setTimeout(() => { setResults(MOCK_RESULTS); setCurrentIdx(0); setSelections({}); setNotes({}); setView('clauses') }, 1500)
  }
  ...
  const handleSendApproval = () => {
    fetch(API + '/api/approvals/templates', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: 'default', name: '默认审批流', nodes: approvalNodes, method: 'sequential', timeout_hours: 48 }) })
      ...
      .then(r => r.json()).then(d => {
        setApprovalInst(d); setView('approval'); saveContract()
        setContracts(prev => prev.map(c => c.id === activeId ? { ...c, approval_status: d.status, approval_id: d.id } : c))
      })
  }
      ...
      <div className="sidebar">
        ...
        <div className="contract-list">
          {contracts.map(c => {
            ...
            return (
              <div key={c.id} className={'contract-item' + (c.id === activeId ? ' active' : '')} onClick={() => { setActiveId(c.id); setView('upload'); setContractText(''); setFile(null) }}>
                <div>{c.name}</div>
        ...
        <div className="main">
          {(view === 'clauses' || view === 'approval') && (
            <div className="panel-left" ref={leftRef}>
              {view === 'clauses' && clause ? makeHighlightedText(contractText || MOCK_CONTRACT_TEXT, clause.id) : (contractText || MOCK_CONTRACT_TEXT)}
            </div>
          )}
            ...
            {activeId && view === 'upload' && (
              <div className="upload-area">
                ...
                <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end' }}>
                  <button className="btn btn-primary" onClick={handleSubmit} disabled={!contractText && !file}>Отправить на AI анализ</button>
                </div>
              </div>
            )}
            {view === 'analyzing' && (
              ...
              <div>
                <div className="ai-card">
                  ...
                  <div style={{ fontSize: 13, lineHeight: 1.8 }}>
                    <span style={{ color: '#fecaca', fontWeight: 700 }}>🔴 {results.filter(r => r.risk === 'high').length} высокий</span> · {' '}
                    <span style={{ color: '#fde68a', fontWeight: 700 }}>🟡 {results.filter(r => r.risk === 'med').length} средний</span>
                  </div>
                </div>
                <div className="nav-row">
                  <span className="counter">{currentIdx + 1} / {results.length}</span>
                </div>
                <div className="clause-card">
                  ...
                  <div className="confirm-row">
                    <button className="btn btn-success" onClick={handleConfirm} disabled={!sel}>
                      {isLast ? 'Завершить и отправить на согласование' : 'Подтвердить → Следующий пункт'}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="approval-section">
                ...
                <div className="approval-steps">
                  {approvalNodes.map((n, i) => [
                    i > 0 ? <span key={'a' + i} className="arrow">→</span> : null,
                    <div key={n.order} className={'approval-step' + (approvalInst.status === 'approved' && i < approvalInst.current_step ? ' done' : approvalInst.status === 'rejected' && i === 0 ? ' rejected' : i === approvalInst.current_step ? ' current' : '')}>
                      <div style={{ fontWeight: 600 }}>{n.approver_name}</div>
                      <div style={{ fontSize: 10, color: '#999' }}>{n.node_type === 'cc' ? 'Копия' : 'Соглас.'}</div>
                    </div>
                  ]).flat()}
                </div>
                {approvalInst.status === 'in_progress' && (
                  <div className="approval-actions">
                    <button className="btn btn-success" onClick={handleApprove}>✅ Утвердить</button>
                    <button className="btn btn-danger" onClick={() => setRejectReason('')}>❌ Отклонить</button>
                  </div>
                )}
                {rejectReason === '' && approvalInst.status === 'in_progress' && (
                  <div className="reject-box" style={{ marginTop: 12 }}>
                    <textarea placeholder="Причина отклонения" value={rejectReason} onChange={e => setRejectReason(e.target.value)} />
                    <button className="btn btn-danger" style={{ marginTop: 8 }} onClick={handleReject} disabled={!rejectReason}>Отклонить</button>
                  </div>
                )}
                {approvalInst.status === 'rejected' && (
                  <div style={{ marginTop: 16, background: '#fef2f2', padding: 12, borderRadius: 6, fontSize: 13 }}>
                    <strong>Причина: </strong>{approvalInst.reject_reason || ''}
                    <div style={{ marginTop: 12 }}><button className="btn btn-primary" onClick={() => { fetch(API + '/api/approvals/instances/' + approvalInst.id + '/resubmit', { method: 'POST' }).then(r => r.json()).then(d => { setApprovalInst(d); setView('clauses') }) }}>Повторно отправить</button></div>
                  </div>
                )}
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            {approvalNodes.map((n, i) => (
              <div key={i} className="step-editor">
                <span style={{ fontSize: 13, color: '#999', minWidth: 40 }}>{i + 1}</span>
                <select value={n.node_type} onChange={ev => { const ns = [...approvalNodes]; ns[i].node_type = ev.target.value; setApprovalNodes(ns) }}>
                  <option value="approver">Согласующий</option>
                  <option value="cc">Копия</option>
                </select>
                <select value={n.approver_id} onChange={ev => { const ns = [...approvalNodes]; ns[i].approver_id = ev.target.value; ns[i].approver_name = USERS.find(u => u.id === ev.target.value)?.name || ''; setApprovalNodes(ns) }}>
                  {USERS.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                </select>
                <button onClick={() => setApprovalNodes(approvalNodes.filter((_, j) => j !== i))}>×</button>
              </div>
            ))}
            ...
            <div className="modal-actions">
              ...
              <button className="btn btn-primary" onClick={() => setShowSettings(false)}>Сохранить</button>
            </div>
          </div>
        </div>
      )}
